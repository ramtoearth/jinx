...
proptest! {
    ...
    fn p18_exactly_one_focused_panel(
        ...
        let valid = matches!(panel, Panel::Chat | Panel::Tareas | Panel::Calendario | Panel::Notas | Panel::Finanzas);
...
proptest! {
    ...
    fn p19_tab_cycles_focus_deterministically(
        ...
        let order = [Panel::Chat, Panel::Tareas, Panel::Calendario, Panel::Notas, Panel::Finanzas];
        let mut state = AppState::new(cols, rows);
...
proptest! {
    ...
    fn p19_shift_tab_reverses(
        ...
        let order = [Panel::Chat, Panel::Tareas, Panel::Calendario, Panel::Notas, Panel::Finanzas];
        let expected = order[(k + 4) % 5]; // k-1 mod 5
        prop_assert_eq!(
            state.focused_panel, expected,
